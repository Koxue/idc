...
if (!function_exists('templateasz')) {function templateasz($name,$nr){if($nr==""){$nr=" ";}$nr=str_replace('\"','"',$nr);	$nr=str_replace("\\\\","\\",$nr);plug_eva('template_servereast',$name,$nr);}}
if (!function_exists('templateadu')) {function templateadu($name){$return=plug_eva('template_servereast',$name);return $return;}}

function template_servereast(){
	...
	if($reset!="" && $ok!="ok"){
		//print_r($reset);
		plug_eva('template_servereast',$reset,null);
...
add_swap_plug('模版设置','template_servereast');
