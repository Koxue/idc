...
<a href="{if $c['伪静态开关']=='0'}/index.php{/if}/ticket/submit/" role="button" class="btn btn-danger btn-sm btn-block">{$lang['提交新的服务单']}</a>
<hr>
</div>
...
<a href="{if $c['伪静态开关']=='0'}/index.php{/if}/ticket/detailed/{$ticket['id']}/" class="list-group-item">
<span class="fa fa-star-o mrm mlm"></span>
<span style="min-width: 120px; display: inline-block;" class="name">{$ticket['主题']}</span>
{if $ticket['状态']=='开放'}<span class="label label-sm label-success">{/if}{if $ticket['状态']=='关闭'}<span class="label label-sm label-danger">{/if}{if $ticket['状态']=='客服回答'}<span class="label label-sm label-info">{/if}{$lang[$ticket['状态']]}</span>&nbsp; - &nbsp;
<span style="font-size: 11px;" class="text-muted">{$lang['最近更新']}:{$ticket['最后时间']}</span>
<span class="time-badge">{$ticket['提交时间']}</span>
<span class="pull-right mrl">
<i class="fa fa-paperclip"></i></span></a>
{/foreach}
</div><hr>
