...
class RenrenSDK extends SWAPOauth{
	/**
	 * 获取requestCode的api接口
	 * @var string
	 */
	...
	protected function parseToken($result, $extend){
		...
		if($data['access_token'] && $data['expires_in'] && $data['refresh_token'] && $data['user']['id']){
			...
			swap_main_error_module('SWAPOauth错误:',"获取人人网ACCESS_TOKEN出错：{$data['error_description']}");
	}
	...
	public function openid(){
		...
		if(!empty($data['openid']))
			...
			swap_main_error_module('SWAPOauth错误:','没有获取到人人网用户ID！');
	}
}