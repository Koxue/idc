...
class QqSDK extends SWAPOauth{
	...
	protected function parseToken($result, $extend){
		...
		if($data['access_token'] && $data['expires_in']){
			...
			swap_main_error_module('SWAPOauth错误:',"获取腾讯QQ ACCESS_TOKEN 出错：{$result}");
	}
	...
	public function openid(){
		...
		elseif($data['access_token']){
			...
			if(isset($data['openid']))
				...
				swap_main_error_module('SWAPOauth错误:',"获取用户openid出错：{$data['error_description']}");
		} else {
			swap_main_error_module('SWAPOauth错误:','没有获取到openid！');
		}
	}
}