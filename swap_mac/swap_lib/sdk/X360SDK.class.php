...
class X360SDK extends SWAPOauth{
	/**
	 * 获取requestCode的api接口
	 * @var string
	 */
	...
	protected function parseToken($result, $extend){
		...
		if($data['access_token'] && $data['expires_in'] && $data['refresh_token']){
			...
			swap_main_error_module('SWAPOauth错误:',"获取360开放平台ACCESS_TOKEN出错：{$data['error']}");
	}
	...
	public function openid(){
		...
		if(!empty($data['id']))
			...
			swap_main_error_module('SWAPOauth错误:','没有获取到360开放平台用户ID！');
	}
